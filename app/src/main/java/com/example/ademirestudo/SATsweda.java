...
public class SATsweda extends AsyncTask<Void,Void,Void> {
    MainActivity mainActivity = new MainActivity();
    private final Java_DLLSAT dllsat;
    protected static SATsweda instance = null;

    SATsweda(Context context) {
        dllsat = new Java_DLLSAT();
    }

    public static synchronized SATsweda getInstance(Context context){
        if (instance == null) {
            instance = new SATsweda(context);
        }
        return instance;
    }
    ...
    public void executeCommand(){
       // mainActivity.statuSat =0;
        ...
        try
        {
            ...
            if (mExitValue!=0 ) {
                Process mIpAddrProcess1 = runtime1.exec("/system/bin/ping -c 1 172.16.0.5");
                mExitValue1 = mIpAddrProcess1.waitFor();
            }


          
            if(mExitValue==0 || mExitValue1==0  ){
